...
/*
 ...
 "Membre2": {
 "posts": 2016,
 "inscription": "23/03/1965"
 }
 };
 // console.log(obj);
 var a = JSON.stringify(obj); // json => string
 console.log(a);
 console.log(JSON.parse(a));  // string => json
 */
...
function loadFile(file) {
    ...
    xhr.addEventListener('readystatechange', function () { // On gère ici une requête asynchrone
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) { // Si le fichier est chargé sans erreur
            ...
            document.getElementById('fileContent').innerHTML = '<span class="error">' + 'Une erreur est survenue !</span><br>(Code: ' + xhr.status + ' -  Texte: ' + xhr.statusText + ')'; // Et on affiche !
